...
describe("project route paths", () => {
  it("creates project detail routes with optional attachment-tab deep-link params", () => {
    expect(createProjectDetailRoute(42)).toBe("/pm/pm/project?projectId=42");
    expect(createProjectDetailRoute(42, { tab: "attachments" })).toBe(
      "/pm/pm/project?projectId=42&tab=attachments",
    );
    expect(
      createProjectDetailRoute(42, { attachmentId: "file-777", tab: "attachments" }),
    ).toBe("/pm/pm/project?projectId=42&tab=attachments&attachmentId=file-777");
    expect(createProjectDetailRoute(42, { attachmentId: "file-777" })).toBe(
      "/pm/pm/project?projectId=42&tab=attachments&attachmentId=file-777",
    );
  });

  it("creates issue detail routes with optional comment permalinks", () => {
    expect(createProjectIssueRoute(42, 7)).toBe("/pm/pm/project/issue?projectId=42&id=7");
    expect(
      createProjectIssueRoute(42, 7, { commentId: 99, tab: "comments" }),
    ).toBe("/pm/pm/project/issue?projectId=42&id=7&tab=comments&commentId=99");
  });

  it("exposes a global mgr-uploads SPA path without project scope", () => {
    expect(PROJECT_MANAGER_MGR_UPLOADS_ROUTE_PATH).toBe("/pm/pm/mgr-uploads");
  });

  it("creates task detail routes and encodes task ids safely", () => {
    expect(createProjectTaskRoute(42, "task-7")).toBe("/pm/pm/project/task?projectId=42&id=task-7");
    expect(
      ...
      "/pm/pm/project/task?projectId=42&id=phase%2Fone+task&tab=comments&commentId=99",
    );
  });
});
