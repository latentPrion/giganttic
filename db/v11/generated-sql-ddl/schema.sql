...
CREATE TABLE `Issues` (
	...
	`name` text NOT NULL,
	`description` text,
	`reusable-v7-issues-journal` text,
...
CREATE INDEX `Notifications_commentId_mentionedUserId_idx` ON `Notifications` (`commentId`,`mentionedUserId`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `ProjectGanttCharts_projectId_chartId_unique` ON `ProjectGanttCharts` (`projectId`,`chartId`);--> statement-breakpoint
...
CREATE TABLE `Projects` (
	...
	`name` text NOT NULL,
	`description` text,
	`reusable-v7-projects-journal` text,
	`createdAt` integer DEFAULT (CAST(unixepoch('subsec') * 1000 AS INTEGER)) NOT NULL,
	`updatedAt` integer DEFAULT (CAST(unixepoch('subsec') * 1000 AS INTEGER)) NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE `TaskAttachments` (
	...
	FOREIGN KEY (`attachmentId`) REFERENCES `Attachments`(`id`) ON UPDATE cascade ON DELETE cascade,
	FOREIGN KEY (`projectGanttChartId`,`taskId`) REFERENCES `TaskMirror`(`projectGanttChartId`,`taskId`) ON UPDATE cascade ON DELETE cascade
);
--> statement-breakpoint
...
CREATE INDEX `Users_Notifications_notificationId_idx` ON `Users_Notifications` (`notificationId`);--> statement-breakpoint
