...
describe("auth and access v3 zod schemas", () => {
  ...
  it("accepts split role assignments", () => {
    ...
    const projectOwnerRole = usersProjectsProjectRolesInsertSchema.parse({
      projectId: 11,
      roleCode: projectRoleCodes.projectOwner,
      userId: 2,
    });

    expect(systemRole.roleCode).toBe("GGTC_SYSTEMROLE_ADMIN");
    expect(projectRole.roleCode).toBe("GGTC_PROJECTROLE_PROJECT_MANAGER");
    expect(projectOwnerRole.roleCode).toBe("GGTC_PROJECTROLE_PROJECT_OWNER");
  });

  it("exports the owner role code constant", () => {
    expect(projectRoleCodes.projectOwner).toBe(
