...
describe("generated sqlite ddl for v4", () => {
  ...
  it("contains executable SQL statements for v4", async () => {
    const statements = await readGeneratedSqlStatements(SCHEMA_VERSION);

    expect(statements.length).toBeGreaterThan(0);
    expect(
      statements.some((statement) => statement.includes("CREATE TABLE `Users_ScopedAccessTokenCredentials`")),
    ).toBe(true);
    expect(getGeneratedSqlDdlDir(SCHEMA_VERSION)).toContain(
      "db/v4/generated-sql-ddl",
    );
  ...
  it("applies cleanly and creates v4 scoped access tables", async () => {
    const tempDir = await createDbTestTempDir("giganttic-v4-ddl-");
    const outputPath = createDbTestExecutionPath(
      tempDir,
      "v4.sqlite",
      dbTestRuntimeConfig,
      "v4 sqlite-ddl test database",
    );
    ...
    const scopedTableCount = querySingleValue(
      db,
    ...
    expect(scopedTableCount).toBe(3);
    db.close();
  });
