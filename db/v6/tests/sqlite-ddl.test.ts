...
describe("generated sqlite ddl for v6", () => {
  ...
  it("contains executable SQL statements for v6", async () => {
    const statements = await readGeneratedSqlStatements(SCHEMA_VERSION);
    ...
    expect(
      statements.some((statement) =>
        statement.includes("CREATE TABLE `TaskMirror`"),
      ),
    ).toBe(true);
    expect(
      statements.some((statement) =>
        statement.includes("CREATE TABLE `TaskComments`"),
      ),
    ).toBe(true);
    expect(getGeneratedSqlDdlDir(SCHEMA_VERSION)).toContain(
      "db/v6/generated-sql-ddl",
    );
  ...
  it("applies cleanly and creates v6 issue + task discussion tables", async () => {
    ...
    const taskTableCount = querySingleValue(
      db,
      "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN (?, ?, ?, ?)",
      [
        "TaskMirror",
    ...
    expect(taskTableCount).toBe(4);
    db.close();
  });

  it("enforces scoped token object composite uniqueness", async () => {
    const db = await createV6Database();
