...
async function exportProjectJournals(db, projectRoot) {
  const rows = db.prepare(
    `SELECT id, "reusable-v7-projects-journal" AS journal
     FROM Projects
     WHERE "reusable-v7-projects-journal" IS NOT NULL`,
  ).all();
...
async function exportIssueJournals(db, projectRoot) {
  const rows = db.prepare(
    `SELECT id, projectId, "reusable-v7-issues-journal" AS journal
     FROM Issues
     WHERE "reusable-v7-issues-journal" IS NOT NULL`,
  ).all();
