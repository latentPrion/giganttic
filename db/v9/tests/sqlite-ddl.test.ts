...
describe("generated sqlite ddl for v9", () => {
  ...
  it("contains executable SQL statements for v9", async () => {
    const statements = await readGeneratedSqlStatements(SCHEMA_VERSION);
    ...
    expect(getGeneratedSqlDdlDir(SCHEMA_VERSION)).toContain("db/v9/generated-sql-ddl");
    expect(getGeneratedSqlDdlFilePath(SCHEMA_VERSION)).toContain(
      "db/v9/generated-sql-ddl/schema.sql",
    );
  });

  it("applies cleanly and creates v9 notification and mention tables", async () => {
    const tempDir = await createDbTestTempDir("giganttic-v9-ddl-");
    const outputPath = createDbTestExecutionPath(
      tempDir,
      "v9.sqlite",
      dbTestRuntimeConfig,
      "v9 sqlite-ddl test database",
    );
  ...
  it("enforces mention uniqueness per logical container and mentioned user", async () => {
    const db = await createV9Database();
  ...
  it("cascades user notification deliveries when the parent notification is deleted", async () => {
    ...
    db.exec("INSERT INTO Projects (id, name) VALUES (1, 'Apollo');");
    db.exec(
      "INSERT INTO Notifications (id, eventType, actorUserId, projectId, issueId, taskId, commentId, attachmentId, mentionedUserId, message, targetUrl) VALUES (10, 'NOTIFICATION_EVENT_PROJECT_JOURNAL_MENTIONED', 1, 1, NULL, NULL, NULL, NULL, 2, 'alice mentioned you in the journal for Project \"Apollo\".', '/pm/project?projectId=1#project-journal');",
    );
    db.exec(
